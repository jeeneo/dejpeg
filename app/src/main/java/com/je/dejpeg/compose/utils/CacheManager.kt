...
object CacheManager {
    ...
    suspend fun saveUnprocessedImage(context: Context, imageId: String, uri: Uri) = withContext(Dispatchers.IO) {
        try {
            val extension = getFileExtension(context, uri)
            val file = File(context.cacheDir, "${imageId}${UNPROCESSED_SUFFIX}.${extension}")
            if (file.exists()) {
                Log.d(TAG, "Unprocessed image already exists for given imageId ($imageId), skipping save: ${file.name}")
                return@withContext
            }
