...
# Defaults
TARGET_ABI="arm64-v8a"; BUILD_TYPE="release"; USE_UPX=true; BUILD_VARIANT="lite"; SIGN_APK=false
...
cleanup() {
    if [[ -n "$UPX_DIR" && -d "$UPX_DIR" ]]; then
        echo "deleting UPX"
        rm -rf "$UPX_DIR" upx-*.tar.xz 2>/dev/null || true
    fi
    if [[ -d "./buildtemp" ]]; then
...
# Force release mode when signing, regardless of other flags
if [[ "$SIGN_APK" == "true" ]]; then
    [[ "$BUILD_TYPE" == "debug" ]] && log "Ignoring --debug flag: --sign requires release mode"
    BUILD_TYPE="release"
fi

# Validate keystore configuration for signing
if [[ "$SIGN_APK" == "true" ]]; then
...
# grab NDK from local.properties
...
if $USE_UPX; then
    if command -v upx &>/dev/null; then UPX_BIN="upx"
        ...
        if [[ "${r:-Y}" =~ ^[Yy]$ ]]; then
            curl -L "$UPX_URL" | tar -xJ -C ./buildtemp && UPX_DIR="./buildtemp/upx-5.1.0-amd64_linux" && UPX_BIN="$UPX_DIR/upx"
...
setup_onnx_runtime() {
    ...
    unzip -q "$tmp/onnx.aar" -d "$tmp" && cp "$tmp/jni/$abi/libonnxruntime.so" "$dst/"
    
    # UPX compress ONNX Runtime
    if $USE_UPX && [[ -n "$UPX_BIN" ]]; then
